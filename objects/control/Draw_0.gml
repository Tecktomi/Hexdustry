	...
	if editor_menu = 1 and not mision_choosing_coord{
		...
		//Opciones generales
			...
			if oleadas{
				...
				xpos = draw_text_xpos(xpos, ypos, "Tiempo primera ronda: ")
				oleadas_tiempo_primera = draw_boton_text(xpos, ypos, oleadas_tiempo_primera)
				xpos = room_width / 2 + 40
				ypos += text_y
				xpos = draw_text_xpos(xpos, ypos, "Tiempo entre rondas: ")
	...
	//Editar Mapa
		...
		for(var i = 0; i < array_length(editor_instrucciones); i++){
			...
			//Ruido Aleatorio
				...
				instruccion[2] = draw_boton_text_list(xpos, ypos, dat2, terreno_nombre)
				xpos += text_x
				xpos = draw_text_xpos(xpos, ypos, " el ")
				instruccion[3] = draw_boton_text(xpos, ypos, dat3)
				xpos += text_x
				xpos = draw_text_xpos(xpos, ypos, "% del tiempo")
			}
			//Menas de Recursos
			else if tipo = 3{
				xpos = draw_text_xpos(xpos, ypos, "Añadir ")
		...
		ypos += text_y
		xpos = draw_text_xpos(xpos, ypos, "Añadir ")
		var a = draw_boton_text_list(xpos, ypos, 0, ["...", "Bloques de Terreno", "Bordes", "Ruido", "Menas de Recursos"])
		if a > 0{
...
if not pausa and temp_hexagono != noone and flag and not (show_menu and edificio_nombre[show_menu_build.index] = "Procesador"){
	...
	if edificio_bool[# mx, my]{
		...
		if edificio_armas[index]{
			...
			if array_length(enemigos) > 0 and edificio.target != null_enemigo
	...
	for(var a = 0; a < array_length(edificios_activos); a++){
			...
			if in(var_edificio_nombre, "Torre básica", "Rifle", "Mortero", "Lanzallamas"){
				...
				if edificio.link = null_edificio{
					...
					if edificio.link.vida <= 0 or edificio.link.vida >= edificio_vida[edificio.link.index]{
						...
						edificio.link.vida = min(edificio.link.vida + 2 * red_power, edificio_vida[edificio.link.index])
		...
		//Fábrica de Drones
			...
			if edificio.select >= 0 and array_length(drones_aliados) < 8{
				flag = false
				size = array_length(dron_precio_id[edificio.select])
				for(var b = 0; b < size; b++)
	...
	for(var a = 0; a < size; a++){
		...
		if aa != enemigo.posa or bb != enemigo.posb{
			...
			if clamp(round(aa / 6), 0, ds_grid_width(chunk_enemigos) - 1) != enemigo.chunk_x or clamp(round(bb / 12), 0, ds_grid_height(chunk_enemigos) - 1) != enemigo.chunk_y{
				...
				temp_array = ds_grid_get(chunk_enemigos, enemigo.chunk_x, enemigo.chunk_y)
