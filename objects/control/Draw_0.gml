...
if menu = 0{
	...
	for(var a = 0; a < array_length(idiomas); a++)
		if draw_sprite_boton(spr_bandera, 20 + 80 * a, 20, 64, 64,,, a){
			idioma = a
...
#region Dibujo
	...
	if enciclopedia > 0{
		...
		//Detalles Edificio
			...
			if edificio_armas[ei] and edificio_nombre[ei] != "Onda de Choque"
...
if not pausa and temp_hexagono != noone and flag and not (show_menu and edificio_nombre[show_menu_build.index] = "Procesador"){
	...
	if edificio_bool[# mx, my]{
		...
		else if var_edificio_nombre = "Tubería Subterránea" and edificio.link != null_edificio{
			draw_set_color(c_blue)
...
if build_index > 0 and win = 0{
	...
	if comprable and temp_hexagono != noone{
		...
		//Sí se puede construir
			...
			if edificio_camino[build_index] or in(var_edificio_nombre, "Tubería", "Muro"){
				//Iniciar arrastre
				if mouse_check_button_pressed(mb_left){
					mx_clic = mx
					my_clic = my
				...
				//Mostrar caminos solos
					...
					if not in(var_edificio_nombre, "Tubería", "Muro"){
						draw_set_color(c_black)
						draw_arrow_off(aa, bb, temp_complex_3.a, temp_complex_3.b, 8)
					}
					if not in(var_edificio_nombre, "Cinta Transportadora", "Cinta Magnética", "Tubería", "Muro"){
			...
			//Vista previa no caminos
				...
				if in(var_edificio_nombre, "Túnel", "Túnel salida"){
					...
					else if var_edificio_nombre = "Tubería Subterránea"{
...
//Destruir edificio
	...
	clicked = false
	if ((mouse_check_button(mb_right) and prev_change) or mouse_check_button_pressed(mb_right)) and temp_hexagono != noone and edificio_bool[# mx, my] and edificio.index != 0{
		if edificio_bool[# mx, my]
			prev_change = true
		delete_edificio(mx, my)
	}
...
if not pausa{
	...
	for(var a = 0; a < array_length(edificios_activos); a++){
		...
		//Acción de torres
			...
			else if var_edificio_nombre = "Onda de Choque"{
				if edificio.proceso < edificio_proceso[index]{
					change_energia(edificio_energia_consumo[index], edificio)
					edificio.proceso += red_power
				}
	...
	for(var a = array_length(enemigos); a > 0; a--){
		...
		for(var b = array_length(temp_array); b > 0; b--){
			...
			if dis < temp_dron_size{
				var aaa = vel * sign(aa - temp_enemigo.a), bbb = vel * sign(bb - temp_enemigo.b)
				dron.a += aaa
				dron.b += bbb
				temp_enemigo.a -= aaa
				temp_enemigo.b -= bbb
	...
	for(var a = array_length(drones_aliados); a > 0; a--){
		...
		if dron.index = 1{
			if array_length(puerto_carga_array) > 0{
				if dron.modo = 0{
					...
					if dis > dron_alcance[dron.index]{
						dis = sqrt(dis)
						dron.a += vel * (edificio.x - aa) / dis
						dron.b += vel * (edificio.y - bb) / dis
					}
					else{
						if dron.modo = 1{
		...
		for(var b = a + 1; b < size_2; b++){
			...
			if dis < 400{//20^2
				var aaa = vel * (aa - temp_dron.a) / dis, bbb = vel * (bb - temp_dron.b) / dis
				dron.a += aaa
				dron.b += bbb
				temp_dron.a -= aaa
				temp_dron.b -= bbb
...
if win > 0{
	...
	if win_step > 25{
		...
		draw_set_font(ft_letra)
		timer -= vel
		var ypos = 200, sec = floor(timer / 60 * vel)
