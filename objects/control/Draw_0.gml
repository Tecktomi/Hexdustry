	...
	for(var a = 0; a < xsize; a++)
		for(var b = 0; b < ysize; b++){
			...
			if temp_terreno.edificio_draw{
				...
				//Dibujo caminos
				if edificio_camino[temp_edificio.index] or edificio_nombre[temp_edificio.index] = "Túnel"{
					...
					//Dibujo de bateria
					else if in(edificio_nombre[temp_edificio.index], "Batería")
					...
					//Dibujo bomba
					else if in(edificio_nombre[temp_edificio.index], "Bomba Hidráulica"){
...
if temp_hexagono != noone and flag{
	...
	if temp_terreno.ore >= 0
		temp_text += $"{rss_name[ore_recurso[temp_terreno.ore]]}: {temp_terreno.ore_amount}\n"
	temp_text += "\n"
	if temp_terreno.edificio_bool{
		...
		if temp_edificio.carga_total > 0{
			...
			for(var a = 0; a < rss_max; a++)
				if temp_edificio.carga[a] > 0
					temp_text += $"  {rss_name[a]}: {temp_edificio.carga[a]}\n"
		}
		//Mostrar recursos subterraneos
		if in(edificio_nombre[temp_edificio.index], "Taladro", "Taladro Eléctrico")
			if temp_edificio.idle
				...
				for(var a = 0; a < ds_list_size(temp_edificio.coordenadas); a++){
					...
					else if terreno_rss[temp_terreno_2.terreno] and in(edificio_nombre[temp_edificio.index], "Taladro Eléctrico")
						temp_array[terreno_rss_id[temp_terreno_2.terreno]] = -1
				}
				for(var a = 0; a < rss_max; a++)
					if temp_array[a] > 0
						temp_text_2 += $"  {rss_name[a]}: {temp_array[a]}\n"
			...
			if edificio_input_all[temp_edificio.index]
				...
				for(var a = 0; a < rss_max; a++)
					if temp_edificio.carga_max[a] > 0
						temp_text_2 += $"  {rss_name[a]}: {temp_edificio.carga_max[a]}\n"
				if temp_text_2 != ""
					temp_text += $"Acepta:\n{temp_text_2}"
			}
		}
		...
		temp_text += $"Porceso: {temp_edificio.proceso}/{edificio_proceso[temp_edificio.index]}\n"
		temp_text += $"Carga total: {temp_edificio.carga_total}\n"
	...
	if temp_hexagono != noone{
		...
		if edificio_camino[build_index]{
			...
			//Mostrar caminos solos
				...
				if not in(edificio_nombre[build_index], "Cinta Transportadora"){
					draw_sprite_off(edificio_sprite[build_index], 0, temp_complex_2.a, temp_complex_2.b, , , (build_dir - 1) * 60, , 0.5)
		...
		//Vista previa tuneles
			...
			if edificio_nombre[build_index] = "Túnel"{
			...
			//Vista previo edificios no arrastrables
				...
				if in(edificio_nombre[build_index], "Taladro", "Taladro Eléctrico"){
					for(var a = 0; a < rss_max; a++){
						temp_array[a] = 0
						temp_array_2[a] = 0
					}
					...
					for(var a = 0; a < ds_list_size(build_list); a++){
						...
						if aa >= 0 and bb >= 0 and aa < xsize and bb < ysize{
							...
							else if terreno_rss[temp_terreno_2.terreno] and in(edificio_nombre[build_index], "Taladro Eléctrico"){
				...
				if in(edificio_nombre[build_index], "Bomba Hidráulica"){
					var flag_2 = true
					for(var a = 0; a < ds_list_size(build_list); a++){
						var temp_complex_3 = build_list[|a]
						var aa = temp_complex_3.a, bb = temp_complex_3.b
		...
		function f1(build_index, build_dir, mx, my){
			...
			for(var a = 0; a < ds_list_size(build_list); a++){
				...
				//Checkear coliciones
				if aa < 0 or bb < 0 or aa >= xsize or bb >= ysize or (temp_terreno.edificio_bool and not (edificio_camino[build_index] and edificio_camino[temp_terreno.edificio.index])) or (not in(edificio_nombre[build_index], "Bomba Hidráulica") and in(terreno_name[temp_terreno.terreno], "Agua", "Agua Profunda")){
					flag = false
					break
				}
				//Reemplazar caminos
				...
				//Checkear minerales
				if in(edificio_nombre[build_index], "Taladro", "Taladro Eléctrico") and temp_terreno.ore >= 0
					flag_2 = true
				//Checkear minerales
				if in(edificio_nombre[build_index], "Taladro Eléctrico") and terreno_rss[temp_terreno.terreno]
					flag_2 = true
				//Checkear agua
				if in(edificio_nombre[build_index], "Bomba Hidráulica") and not in(terreno_name[temp_terreno.terreno], "Agua", "Agua Profunda")
					flag = false
			}
			if in(edificio_nombre[build_index], "Taladro", "Taladro Eléctrico") and not flag_2
				flag = false
			if edificio_nombre[build_index] = "Túnel" and build_able and build_target.index = 6
				build_index = 16
			if flag
...
for(var a = 0; a < ds_list_size(edificios); a++){
	...
	if not temp_edificio.idle{
		//Accion taladro
		if in(edificio_nombre[temp_edificio.index], "Taladro", "Taladro Eléctrico") and temp_edificio.carga_total < edificio_carga_max[temp_edificio.index]{
			if edificio_electricidad[temp_edificio.index]{
			...
			if temp_edificio.proceso >= edificio_proceso[temp_edificio.index]{
				...
				while not ds_list_empty(temp_list){
					...
					else if terreno_rss[temp_terreno.terreno] and in(edificio_nombre[temp_edificio.index], "Taladro Eléctrico"){
		...
		//Accion caminos
		if (edificio_camino[temp_edificio.index] or edificio_nombre[temp_edificio.index] = "Túnel") and temp_edificio.carga_total > 0 and not temp_edificio.waiting{
			temp_edificio.proceso++
			if temp_edificio.proceso = edificio_proceso[temp_edificio.index]{
		...
		if in(edificio_nombre[temp_edificio.index], "Bomba Hidráulica"){
...
#region Input
	...
	if keyboard_check(ord("L")){
		...
		for(var a = 0; a < ds_list_size(redes); a++){
			...
			temp_text += "  Batería: " + string(floor(temp_red.bateria)) + "/" + string(temp_red.bateria_max) + "\n"
			temp_text += "  Edificios:\n"
