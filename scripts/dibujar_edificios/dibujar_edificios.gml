function dibujar_edificios(){
	with control{
		var xsize2 = (CHUNK_WIDTH * 48 + 8) * zoom, ysize2 = (CHUNK_HEIGHT + 1) * 14 * zoom, xpos = CHUNK_WIDTH * 48 * zoom, ypos = CHUNK_HEIGHT * 14 * zoom
		...
		for(a = min_chunka; a < max_chunka; a++)
			for(b = min_chunkb; b < max_chunkb; b++){
				if chunk_edificios_dirty[# a, b] and array_length(chunk_edificios_estatico[# a, b]) = 0{
					...
					chunk_edificios_background[# a, b] = spr_hexagono
					continue
				}
				if chunk_edificios_dirty[# a, b]{
					...
					sprite = sprite_create_from_surface(background_surface, 0, 0, (CHUNK_WIDTH * 48 + 8), (CHUNK_HEIGHT + 1) * 14, false, false, 0, 0)
					if chunk_edificios_background[# a, b] != spr_hexagono
						sprite_delete(chunk_edificios_background[# a, b])
				...
				camy = prev_camy
				zoom = prev_zoom
				if array_length(chunk_edificios_estatico[# a, b]) > 0
