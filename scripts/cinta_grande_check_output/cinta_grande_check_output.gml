function cinta_grande_check(a, b, dir, index){
	with control{
		...
		//INPUTS
		for(var c = array_length(build_arround) - 1; c >= 0; c--){
			var temp_complex = build_arround[c], aa = temp_complex[0], bb = temp_complex[1], flag = false, flag_2 = false
			...
			if edificio_bool[# aa, bb]{
				...
				if in(edificio.index, id_fabrica_de_drones, id_cinta_grande) and not array_contains(inputs, edificio){
					...
					for(var d = array_length(temp_array) - 1; d >= 0; d--){
						...
						for(var e = array_length(build_size) - 1; e >= 0; e--){
							var temp_complex_2 = build_size[e], aaaa = temp_complex_2[0], bbbb = temp_complex_2[1]
