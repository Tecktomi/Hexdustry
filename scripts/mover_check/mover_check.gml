function mover_check(rss, edificio = control.null_edificio, target = control.null_edificio){
	with control{
		...
		return (array_contains(edificio.outputs, target) and
			...
			not (in(index, id_rifle, id_mortero, id_fabrica_de_drones) and in(rss, id_uranio_enriquecido, id_uranio_empobrecido) and target.carga[id_uranio_bruto] >= target.carga_max[id_uranio_bruto]) and
