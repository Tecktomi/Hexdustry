function mover_check(rss, edificio = control.null_edificio, target = control.null_edificio){
	with control{
		...
		return array_contains(edificio.outputs, target) and (index = id_nucleo or
			((target.carga_total < edificio_carga_max[index] and target.carga[rss] < target.carga_max[rss]) and
