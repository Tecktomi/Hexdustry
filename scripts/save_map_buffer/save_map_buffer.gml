function save_map_buffer(buffer){
	with control{
		...
		for(a = 0; a < terreno_max; a++)
			if a != max_terreno{
				temp_complex = cordenadas[a]
				buffer_write(buffer, buffer_u16, array_length(temp_complex))
				for(b = 0; b < array_length(temp_complex); b++){
					buffer_write(buffer, buffer_u8, temp_complex[b, 0])
					buffer_write(buffer, buffer_u8, temp_complex[b, 1])
				}
			}
		//Recursos
