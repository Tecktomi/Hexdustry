function load_edificio(buffer, edificio = control.null_edificio){
	with control{
		var mask = buffer_read(buffer, buffer_u64), c = 0, temp_target_dron = -1
		...
		if mask & (1 << c++) edificio.start = bool(buffer_read(buffer, buffer_bool))
		...
		if mask & (1 << c++) edificio.waiting = bool(buffer_read(buffer, buffer_bool))
		if mask & (1 << c++) edificio.idle = bool(buffer_read(buffer, buffer_bool))
		...
		if mask & (1 << c++) edificio.luz = bool(buffer_read(buffer, buffer_bool))
		if mask & (1 << c++){
		...
		if mask & (1 << c++) edificio.eliminar = bool(buffer_read(buffer, buffer_bool))
		if mask & (1 << c++) edificio.agregar = bool(buffer_read(buffer, buffer_bool))
		if mask & (1 << c++) edificio.modulo = bool(buffer_read(buffer, buffer_bool))
