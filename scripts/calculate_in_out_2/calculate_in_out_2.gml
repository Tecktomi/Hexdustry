	...
	with control{
		...
		for(var c = 0; c < ds_list_size(temp_list); c++){
			...
			if edificio_bool[# aa, bb]{
				...
				var par_dir = next_to(a, b, dir), par_dir_1 = next_to(a, b, (dir + 1) mod 6), par_dir_5 = next_to(a, b, (dir + 5) mod 6)
				var par_dir_a = next_to(temp_a, temp_b, temp_dir), par_dir_a_1 = next_to(temp_a, temp_b, (temp_dir + 1) mod 6), par_dir_a_5 = next_to(temp_a, temp_b, (temp_dir + 5) mod 6)
				...
				if flag and temp_edificio.receptor and edificio.emisor and ds_list_find_index(edificio.outputs, temp_edificio) = -1{
					...
					if var_edificio_nombre = "Túnel"
						flag = false
					if flag and in(var_edificio_nombre, "Cinta Transportadora", "Cinta Magnética") and not complex_equal(temp_complex, next_to(a, b, dir))
						flag = false
					if flag and in(edificio_nombre[temp_index], "Cinta Transportadora", "Cinta Magnética") and next_to_build(par_dir_a, edificio)
						flag = false
					if flag and in(var_edificio_nombre, "Enrutador", "Selector", "Overflow") and not(complex_equal(temp_complex, par_dir_5) or complex_equal(temp_complex, par_dir) or complex_equal(temp_complex, par_dir_1))
						flag = false
					if flag and in(edificio_nombre[temp_index], "Enrutador", "Selector", "Overflow", "Túnel salida")
				...
				if flag and temp_edificio.emisor and edificio.receptor and ds_list_find_index(edificio.inputs, temp_edificio) = -1{
					flag = true
					if edificio_nombre[temp_index] = "Túnel"
						flag = false
					if flag and in(var_edificio_nombre, "Cinta Transportadora", "Cinta Magnética") and complex_equal(temp_complex, par_dir)
						flag = false
					if flag and in(edificio_nombre[temp_index], "Cinta Transportadora", "Cinta Magnética") and not next_to_build(par_dir_a, edificio)
						flag = false
					...
					if flag and in(edificio_nombre[temp_index], "Túnel salida")
						for(var d = 0; d < ds_list_size(edificio.coordenadas); d++){
							var temp_complex_2 = edificio.coordenadas[|d]
							if complex_equal(par_dir_a_5, temp_complex_2) or complex_equal(par_dir_a, temp_complex_2) or complex_equal(par_dir_a_1, temp_complex_2){
								flag = false
