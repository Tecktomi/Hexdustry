function control_camara(min_camx = 0){
	with control{
		...
		if keyboard_check(ord("D"))
			camx = min(camx + cam_vel + 12 * keyboard_check(vk_lshift), xsize * 48 * zoom - room_width)
		if keyboard_check(ord("S"))
			camy = min(camy + cam_vel + 12 * keyboard_check(vk_lshift), ysize * 14 * zoom - room_height)
		if camx > min_camx and keyboard_check(ord("A"))
			camx = max(camx - cam_vel - 12 * keyboard_check(vk_lshift), min_camx)
		if camy > 0 and keyboard_check(ord("W"))
			camy = max(camy - cam_vel - 12 * keyboard_check(vk_lshift), 0)
	}
}