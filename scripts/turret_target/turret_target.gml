function turret_target(edificio = control.null_edificio, alc_min = 0){
	with control{
		var dis = edificio_alcance_sqr[edificio.index], enemigo_final = null_enemigo, center_x = edificio.center_x, center_y = edificio.center_y
		//Torre enemiga
		if edificio.enemigo{
			...
			if alc_min = 0{
				for(var a = array_length(edificio.target_chunks) - 1; a >= 0; a--){
					var temp_complex = edificio.target_chunks[a], temp_array = chunk_dron_aliado[# temp_complex.a, temp_complex.b]
					for(var b = array_length(temp_array) - 1; b >= 0; b--){
						var enemigo = temp_array[b]
						if enemigo.vida <= 0
							continue
						var temp_dis = distance_sqr(center_x, center_y, enemigo.a, enemigo.b)
						if temp_dis < dis{
							dis = temp_dis
		...
		//Torre aliada
			...
			if alc_min = 0{
				for(var a = array_length(edificio.target_chunks) - 1; a >= 0; a--){
					var temp_complex = edificio.target_chunks[a], temp_array = chunk_dron_enemigo[# temp_complex.a, temp_complex.b]
					for(var b = array_length(temp_array) - 1; b >= 0; b--){
						var enemigo = temp_array[b]
						if enemigo.vida <= 0
							continue
