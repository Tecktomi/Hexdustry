function calcular_edificios_adyascentes(edificio = control.null_edificio, set_receptor = true){
	...
	with control{
		...
		for(i = array_length(edificio.outputs) - 1; i >= 0; i--){
			temp_edificio = edificio.outputs[i]
		...
		//Inputs e Outputs
			...
			for(c = array_length(temp_list) - 1; c >= 0; c--){
				...
				if temp_edificio.jugador != _jugador
					continue
				temp_a = temp_edificio.a
				temp_b = temp_edificio.b
				temp_index = temp_edificio.index
				if temp_index = id_cruce{
					calcular_edificios_adyascentes(temp_edificio)
					continue
				}
				temp_dir = temp_edificio.dir
				par_dir = next_to(a, b, dir)
				par_dir_1 = next_to(a, b, (dir + 1) mod 6)
				par_dir_2 = next_to(a, b, (dir + 2) mod 6)
				par_dir_3 = next_to(a, b, (dir + 3) mod 6)
				par_dir_4 = next_to(a, b, (dir + 4) mod 6)
				par_dir_5 = next_to(a, b, (dir + 5) mod 6)
				par_dir_a = next_to(temp_a, temp_b, temp_dir)
				par_dir_a_1 = next_to(temp_a, temp_b, (temp_dir + 1) mod 6)
				par_dir_a_2 = next_to(temp_a, temp_b, (temp_dir + 2) mod 6)
				par_dir_a_3 = next_to(temp_a, temp_b, (temp_dir + 3) mod 6)
				par_dir_a_4 = next_to(temp_a, temp_b, (temp_dir + 4) mod 6)
				par_dir_a_5 = next_to(temp_a, temp_b, (temp_dir + 5) mod 6)
			...
			for(c = array_length(edificio.outputs) - 1; c >= 0; c--){
				temp_edificio = edificio.outputs[c]
				if temp_edificio.index = id_cinta_transportadora
					camino_calcular_in(temp_edificio)
			}
		}
