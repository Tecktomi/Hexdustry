function mover_in(edificio = control.null_edificio){
	with control{
		var len = array_length(edificio.inputs), moved = false, a, temp_edificio
		for(a = 0; a < len; a++){
			temp_edificio = edificio.inputs[(edificio.input_index + a) mod len]
			...
			if temp_edificio.waiting and temp_edificio.carga_total > 0 and mover(temp_edificio){
				...
				if edificio.carga_total >= edificio_carga_max[edificio.index]{
					edificio.input_index = (edificio.input_index + a + 1) mod len
					return true
				}
			}
		}
