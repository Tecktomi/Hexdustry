	...
	with control{
		...
		if array_length(flujo.edificios) = 0{
			...
			if array_length(flujo_link) > 1{
				...
				for(var a = array_length(flujo_link) - 1; a >= 0; a--){
					var temp_edificio = flujo_link[a]
				...
				while array_length(flujo.edificios) > 0{
					var nodo = flujo.edificios[array_length(flujo.edificios) - 1]
					if not visited[nodo.edificio_index]{
						...
						while not ds_stack_empty(pila){
							...
							if not visited[nodo.edificio_index]{
								...
								var temp_flujo_link = (nodo.flujo = flujo) ? nodo.flujo_link : nodo.flujo_2_link
								for(var a = array_length(temp_flujo_link) - 1; a >= 0; a--){
									var temp_edificio = temp_flujo_link[a]
		...
		var temp_list = get_arround(edificio.a, edificio.b, edificio.dir, edificio_size[index])
