function draw_text_rich(xx, yy, width = room_width - xx, arguments = []){
	...
	var xpos = xx, ypos = yy
	for(var a = 0; a < array_length(arguments); a++){
		var argumento = arguments[a]
		//FUNCIONES
		if is_struct(argumento){
			var param = argumento, funcion = arguments[++a]
			argumento = arguments[++a]
			//DRAW_BOTON
			if is_string(argumento){
				var arg_string = string(argumento), parrafos = string_split(arg_string, "\n")
				var primera_linea = "", trozos = string_split(parrafos[0], " "), spos = 0, xxx = xpos, flag = (string_count("\n", arg_string) = 0)
				for(var i = 0; i < array_length(trozos); i++){
					if xxx > width + xx{
						flag = false
						break
					}
				...
				if flag{
					...
					if draw_boton(xpos, ypos, primera_linea,,,, false, input_layer)
						funcion(param)
					ypos += text_y
					var segunda_linea = text_wrap(string_delete(arg_string, 0, spos), width)
					var ultima_linea = string_delete(segunda_linea, 0, string_last_pos("\n", segunda_linea))
