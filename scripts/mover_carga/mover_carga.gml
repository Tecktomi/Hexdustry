function mover_carga(edificio = control.null_edificio){
	with control{
		...
		//Entregar a outputs
			...
			for(a = 0; a < array_length(edificio.outputs_carga); a++){
				temp_edificio = edificio.outputs_carga[(a + edificio.outputs_carga_index) mod array_length(edificio.outputs_carga)]
		...
		if flag{
			...
			for(a = 0; a < array_length(edificio.inputs_carga); a++){
				temp_edificio = edificio.inputs_carga[a]
