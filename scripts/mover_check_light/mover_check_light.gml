function mover_check_light(rss, target = control.null_edificio){
	with control{
		var index = target.index
		return (index = id_nucleo or ((target.carga_total < edificio_carga_max[index] and target.carga[rss] < target.carga_max[rss]) and
