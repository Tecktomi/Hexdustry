function change_calor(calor, edificio = control.null_edificio){
	with control{
		if calor = edificio.calor_generado
			exit
		var a = calor - edificio.calor_generado
		edificio.calor += 2 * a
		edificio.calor_generado = calor
		for(var i = array_length(edificio.bordes) - 1; i >= 0; i--){
			var temp_complex = edificio.bordes[i], aa = temp_complex[0], bb = temp_complex[1]
