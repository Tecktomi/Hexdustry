function scr_torre_reparadora(edificio = control.null_edificio){
	with control{
		...
		if edificio.link = null_edificio{
			...
			if edificio != edificio.link and edificio.link.vida >= edificio_vida[edificio.link.index] or point_distance(edificio.center_x, edificio.center_y, edificio.link.center_x, edificio.link.center_y) > edificio_alcance[index]{
